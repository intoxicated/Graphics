...
TEST(VectorGet, ValidGet) {
    ...
    for(int i = 1; i <= 5; ++i)
    {
        ...
        EXPECT_TRUE(v.get(i) == expected[i-1]);
    }
}
...
TEST(VectorOrientation, ValidOrientation) {
    ...
    Vector v(5, Vector::ROW), v1(2, Vector::COLUMN);
...
TEST (VectorNorm, ValidNorm3) {
    ...
    Vector v(5, Vector::ROW), v1(5, Vector::ROW);
...
TEST (VectorNormalized, ValidNormalized3) {
    ...
    double expected[] = {v2.get(1)/n, v2.get(2)/n, v2.get(3)/n};
...
TEST(VectorPlusOperator, ValidOperator) {
    ...
    Vector v(3, Vector::COLUMN), v1(3, Vector::COLUMN), result(3, Vector::COLUMN);
    v = { 2,3,4 };
    v1 = { 5,6,7 };
...
TEST(VectorEqualOperator, ValidOperator) {
    ...
    Vector v(4, Vector::ROW), v2(4, Vector::ROW);
